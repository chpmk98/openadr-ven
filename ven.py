...
class VEN(ABC):
    ...
    def _connect_to_full_URL(self, a_full_URL):
        try:
            self.logger.info("Connecting to {}...".format(a_full_URL))
            # For the VTN-RI
            self.vtn = VTNOps(a_full_URL, self.ven["client_id"], self.ven["client_secret"])
        except:
            self.logger.info("Could not connect to {}.".format(a_full_URL))
            self.vtn = None
    ...
    def _attempt_connection(self, a_VTN):
        ...
        if "full URL" in a_VTN:
            ...
            if self.vtn is not None:
                self.logger.info("Successfully connected to {}!".format(a_VTN["full URL"]))
                return
        ...
        for an_address in a_VTN["addresses"]:
            ...
            if self.vtn is not None:
                self.logger.info("Successfully connected to {}!".format(VTN_full_url))
                return
    ...
    def _use_this_VTN(self, name, info):
        ...
        self.logger.info("Local VTN server '{}' found with address(es) {} at port {}.".format(name, info.addresses, info.port))
        ...
        if state_change is ServiceStateChange.Added:
            ...
            if info:
                ...
                if not self._use_this_VTN(name, info):
                    self.logger.info("Rejecting local VTN '{}'. Continuing to look for other local VTNs...")
                    return
                ...
                if self.vtn is None:
                    self.logger.info("Failed connecting to local VTN '{}.' Continuing to look for other local VTNs...")
                    return
    ...
    def _get_desired_program_index(self, program_list):
        self.logger.info("Found {} programs! Which one do you want?".format(program_list.num_programs()))
        for an_ind, a_program in enumerate(program_list, start=1):
            self.logger.info("[{}] Program ID: {}, Program Name: {}, Country: {}, Program Type: {}".format(an_ind, a_program.getId(), a_program["programName"], a_program["country"], a_program["programType"]))
    ...
    def _select_program(self):
        ...
        if self.program_id is not None:
            ...
            if self.program is None:
                self.logger.info("Failed to find a program with ID '{}' on this VTN. Getting a list of all available programs instead..".format(self.program_id))
        ...
        self.logger.info("Using Program ID: {}".format(self.program_id))
    ...
    def run(self):
        ...
        try:
            ...
            if self.advertise:
                self.logger.info("Unregistering VEN...")
